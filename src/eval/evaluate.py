...
from data.data import canonicalize_peptide, decoder_output_token_ids, invert_vocabulary
...
def evaluate_generative(
    ...
    *,
    ...
    if return_details:
        ...
        exact_matches = [
            canonicalize_peptide(p) == canonicalize_peptide(t) for p, t in zip(predictions, targets)
        ]
        exact_matches_il = [
            canonicalize_peptide(p).replace("I", "L") == canonicalize_peptide(t).replace("I", "L")
