...
def _prefix_mass(sequence: str | list[str], end: int) -> float:
    tokens = parse_peptide(sequence) if isinstance(sequence, str) else sequence
    return sum(_residue_mass(amino_acid) for amino_acid in tokens[:end])
...
def count_matching_amino_acids(
    ...
    pred_tokens = parse_peptide(predicted) if isinstance(predicted, str) else list(predicted)
    true_tokens = parse_peptide(target) if isinstance(target, str) else list(target)
    n, m = len(pred_tokens), len(true_tokens)
    if n == 0 or m == 0:
        return 0

    # Fast path: identical token lists — every position matches.
    if pred_tokens == true_tokens:
        return n

    # Pre-compute cumulative prefix masses as numpy arrays (avoids O(N²) Python calls).
    pred_masses = np.array([_residue_mass(aa) for aa in pred_tokens], dtype=np.float64)
    true_masses = np.array([_residue_mass(aa) for aa in true_tokens], dtype=np.float64)
...
def peptide_matches_mass_based(
    ...
    for index in range(len(pred_tokens)):
        if abs(pred_masses[index] - true_masses[index]) >= aa_mass_tolerance:
            return False
        if abs(pred_prefixes[index] - true_prefixes[index]) >= prefix_mass_tolerance:
            return False
    return True
...
def compute_denovo_metrics(
    ...
    for index, (predicted, target) in enumerate(zip(predictions, targets)):
        pred_toks = parse_peptide(predicted) if isinstance(predicted, str) else list(predicted)
        tgt_toks = parse_peptide(target) if isinstance(target, str) else list(target)
        ...
        aa_pred_total += len(pred_toks)
        aa_gold_total += len(tgt_toks)

        pred_canonical = "".join(pred_toks)
        tgt_canonical = "".join(tgt_toks)
        exact_match = (pred_canonical == tgt_canonical)
        exact_match_il = (pred_canonical.replace("I", "L") == tgt_canonical.replace("I", "L"))
